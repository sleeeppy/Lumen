    ...
    public class RetroBeamStatic : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            if (beam)
            {
                ...
                if (beamCollides && Physics.Raycast(transform.position, transform.forward, out hit) && !hit.collider.CompareTag("PlayerBullet"))
                {
                    if (hit.collider.CompareTag("BossBullet"))
                    {
                        DestructibleProjectile destructible = hit.collider.GetComponent<DestructibleProjectile>();
                        if (destructible != null)
                        {
                            destructible.OnHit(Time.deltaTime);
                        }
                        else return;
                    }
